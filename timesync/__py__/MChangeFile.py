...
# Функция замены первой найденой строки которая содержит заданный шаблон
class ReplaceTemplateOneFind(object):
    def __init__(self, file, template, new_str):
        print('Функция замены первой найденой строки которая содержит заданный шаблон... ')
    ...
    def start_func_replace_template(self):
        ...
        with open(tmp_file, 'w') as f1:
            with open(self.file, 'r') as f:
                for line in f:
                    ln = ''.join(line.split())
                    if self.template in ln and self.exit_code == -1:
                        f1.writelines(self.new_str + '\n')
                        self.exit_code = 0
                    else:
                        f1.writelines(line)
